...
class SigLIPDistilBert(nn.Module):
    def __init__(
        ...
        centroid1 = torch.tensor(self.centroids[0], dtype='uint8', device='cuda').unsqueeze(0)
        centroid2 = torch.tensor(self.centroids[1], dtype='uint8', device='cuda').unsqueeze(0)
        # Assume batch["image"] shape: [B, 3, 224, 224], centroids: [3, 224, 224]
        
        centroid1_flat = centroid1.view(1, -1)
        centroid2_flat = centroid2.view(1, -1)
        self.centroids=(centroid1_flat, centroid2_flat)
        ...
        self.img_encoder1 = get_peft_model(self.img_encoder, vision_lora_cfg)
        self.img_encoder2 = get_peft_model(self.img_encoder, vision_lora_cfg)
        ...
        for p in self.img_encoder2.visual.parameters():
            p.requires_grad = False
        # compute the embedding dimension by doing a dummy pass
        with torch.no_grad():
            dummy = torch.zeros(1, 3, 224, 224)
            clip_dim = self.img_encoder2.encode_image(dummy).shape[-1]
    ...
    def forward(self, batch):
        img_flat = batch["image"].to(self.img_encoder1.visual.device)  # [B, 3, 224, 224]
        centroid1, centroid2 = self.centroids
        d1 = torch.norm(img_flat - centroid1, dim=1)
        d2 = torch.norm(img_flat - centroid2, dim=1)
        img_f1 = self.img_encoder1.encode_image(batch["image"])
        img_f2 = self.img_encoder2.encode_image(batch["image"])

        # Compute weights using softmax over negative distances
        dists = torch.stack([d1, d2], dim=1)         # [B, 2]
        weights = F.softmax(-dists, dim=1)           # [B, 2]
        w1 = weights[:, 0:1]                         # [B, 1]
        w2 = weights[:, 1:2]                         # [B, 1]
        ...
        img_mix_in = torch.cat([img_f1, img_f2, w1, w2], dim=1)  # [B, 2D+2]
